...
function Backfun (centerDivId,backgroundColor,electric,blurCol, miSize, maSize) {
  var cdiv = centerDivId;
  var bc = backgroundColor;
  var e = electric;
  var blur = blurCol;
  var minSize = (miSize <= 0 || miSize == undefined) ? 5 : miSize;
  var maxSize = (maSize <= 0 || miSize == undefined) ? 20 : maSize;
  ...
  this.addMouse = function(colArr, showChance) {
    poses.push({
      mouse: true,
      col: colArr,
      show_chance: showChance || 0
    });
    show = true;
    return poses.length-1;
  }
  this.addPoint = function(x,y,colArr, showChance) {
    poses.push({
      mouse: false,
      x: x,
      y: y,
  ...
  this.get = function(index) {
    ...
    var render = function(){
      ...
      ctx.fillStyle = bc;
        ...
        if(e && circles.length > 0){
        	// set the stoke of first line
      ctx.lineWidth = 1;
      ...
      ctx.stroke();
        ...
        for(c in circles){
                c = circles[c];
                drawCircle(ctx, c.color, c.x, c.y, c.s, 0, 0,blur);
        }

        if(show){
          //Use this if you want a constant circle there (a spawning circle almost)
          drawCircle(ctx, "rgba(0,0,0,.5)", mousex, mousey, 10, 0, 0,"rgba(0,0,0,0)")
        }
    }
  ...
  canvas.height = h;
    ...
    this.render = function(){
      ...
      ctx.clearRect(0,0,size,size);
         ...
         for(c in circles){
                c = circles[c];
                drawCircle(ctx, c.color, c.x, c.y, c.s, 0, 0,"rgba(0,0,0,0)");
        }
        // Use this if you want a constant circle there (a spawning circle almost)
