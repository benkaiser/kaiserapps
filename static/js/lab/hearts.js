...
var reset = function () {
	...
	CircleArr[0] = {
		...
		y: h,
		s: h/50,
		color: hslToRgb(currentColor,1,0.5)
	};
};
...
var update = function (modifier) {
	...
	for(var circle in CircleArr){
		...
		circle.x += Math.random()*speed_mod/2-(speed_mod/4);
		circle.y -= Math.random()*speed_mod;
	}
...
function createCircle(){
	...
	CircleArr[CircleArr.length] = {
		...
		y: mousey+30,
		s: Math.random()*h/10,
		color: hslToRgb(currentColor % 1.00,1,0.5)
	};
}
...
var render = function () {
	...
	for(var circle in CircleArr){
		...
		drawHeart(ctx,current.color,current.x,current.y,current.s,10,"#FFF");
	}

};

function drawHeart(ctx, fillColor, x, y, radius, strokeWidth, strokeColor){
	ctx.fillStyle = colorToHex("rgb("+fillColor[0].toFixed(0)+","+fillColor[1].toFixed(0)+","+fillColor[2].toFixed(0)+")");
	//ctx.fillStyle = colorToHex("#FF19AF");
